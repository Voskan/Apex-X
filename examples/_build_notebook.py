...
cells.append(code([
    ...
    "        targets = {\n",
    "            'boxes': torch.cat([torch.from_numpy(s.boxes_xyxy).to(DEVICE) for s in samples]),\n",
    "            'labels': torch.cat([torch.from_numpy(s.class_ids).to(DEVICE) for s in samples]),\n",
    "            'masks': torch.cat([torch.from_numpy(s.masks).to(DEVICE) if s.masks is not None else torch.zeros((len(s.class_ids), 1, 28, 28)).to(DEVICE) for s in samples])\n",
    "        }\n",
    "        \n",
